...
SMMU 寄存器占用了两个连续的 64K 页，SMMU 寄存器页 0 和 SMMU 寄存器页 1，这是架构上要求的。可能会出现用于可选的或 **实现定义** 的特性的附加寄存器页：
...
![用于子流的多上下文描述符](images/1315506-0876894aa8a348cb.png)
...
图 **L1 上下文描述符** 展示了 L1CD 的字段。

![L1 上下文描述符](images/1315506-326f10f81fca7416.png)

L1CD 是 64 位的，主要包含 2 个字段。
...
L1 上下文描述符用于上面提到的 2 级 CD 表。

#### CD 结构

CD 结构包含第 1 阶段转换表指针和关联的字段，如 ASID 和表遍历属性。转换表可能被解释为 AArch32 (Long/LPAE) 或 AArch64 格式 (由 AA64 字段控制)。

无效或矛盾的 CD 配置被标记为 **ILLEGAL**。尝试通过包含 **ILLEGAL** 配置的 CD 进行转换的事务将通过中止而终止，并且将在与事务的安全状态相应的事件队列中 (由 SEC_SID 确定) 记录 **C_BAD_CD** 事件。

下面的列表列出了一些常用字段：

 * **Valid**：CD 是否有效
 * **IPS**：中间物理地址大小
 * **T0SZ**：TT0 (TT，Translation table，转换表) 覆盖的 VA 区域大小
 * **T1SZ**：TT1 覆盖的 VA 区域大小
 * **AA64**：转换表格式，用于确定 **TTB{0,1}** 是指向 AArch32 格式的转换表，还是指向 AArch64 格式的转换表
 * **ASID**：地址空间大小
 * **TTB0**：TT0 基地址
 * **TTB1**：TTB1 基地址
 * **PARTID**：MPAM 分区 ID
...
CD 的详细结构可以在 [ARM 系统内存管理单元架构规范版本 3](https://developer.arm.com/documentation/ihi0070/latest/) 找到。

### 转换表

SMMUv3 第 1 和第 2 阶段转换都支持 AArch32 (LPAE) 和 AArch64 转换表格式。
...
SMMU 通过内存中的一个环形命令队列来控制。例如，当软件更改 STE 或转换时，它需要使 SMMU 中的相关缓存无效。这可以通过向命令队列发出相应的无效命令来完成。请参考 [命令](https://developer.arm.com/documentation/109242/0100/Programming-the-SMMU/Command-queue/Commands?lang=en) 一节了解命令的分类。
...
为向 **与同步命令发出的相同** 命令队列发出的先前的命令提供同步机制。
...
恢复或终止对已停止事务的处理，其中该事务由给定的 StreamID 和 STAG 参数标识。
...
如果发生了配置错误和故障，则会将其记录在事件队列中。其中包括由传入设备的流量产生的事件，例如：
...
如果事件消耗得不够快，由传入事务引起的一系列故障或错误可能会填满事件队列并导致其溢出。如果事件队列已满，则由停滞的故障事务产生的事件永远不会被丢弃。当事件队列中的条目被消耗，并且接下来空间变得可用时，则记录它们。如果事件队列已满，其它类型的事件将被丢弃。系统软件应快速消耗事件队列中的条目，以避免正常操作期间溢出。

#### 事件记录

事件记录的大小为 32 字节，且所有的事件记录都是小尾端的。事件队列可能记录三类事件：
...
与编程接口相关的全局错误将报告到相应的 **SMMU_(\*_)GERROR** 寄存器中，而不是报告到基于内存的事件队列中。这些错误往往很严重，比如阻止 SMMU 向前推进。例如，访问配置数据结构之一时发生外部中止。以下列表展示了所有全局错误：
...
**SMMU_(\*_)GERROR** 为每个全局错误提供了一个一位标记。当错误条件被触发时，SMMU 通过切换 **GERROR** 中的相应标志来激活错误。某些情况下，当错误处于激活状态时，SMMU 行为会发生变化。例如，当命令队列错误处于激活状态时，SMMU 不会从命令队列消耗命令。

当 **SMMU_(\*_)IRQ_CTRL.GERROR_IRQEN == 1**，且当 SMMU 激活非 **GERROR** **MSI** 写入中止的错误时，会触发 **GERROR** 中断。
 ...
 * 通过轮询 **SMMU_CR0ACK** 直到 **CMDQEN** 读取为 1 来检查启用操作完成。
 * 通过将 **SMMU_CR0.EVENTQEN** 位设置为 1 启用事件队列。
 * 通过轮询 **SMMU_CR0ACK** 直到 **EVENTQEN** 读取为 1 来检查启用操作完成。
 ...
 * 或者，安全软件可以通过单次写入使所有 TLB 和缓存失效。
     ...
     * 在继续 SMMU 配置之前，轮询 **SMMU_S_INIT.INV_ALL** 以检查其设置为 0。

7. 启用转换

 * 将 **SMMU_CR0.SMMUEN** 位设置为 1。
 * 通过轮询 **SMMU_CR0ACK** 直到 **SMMUEN** 读取为 1 来检查启用操作完成。

 > **注意**
 > 
 > 此序列展示了当 SMMU 未实现 RME 扩展或未启用 GPC 时的非安全 SMMU 的编程。安全的或 Realm SMMU 编程与此类似。如果 SMMU 实现了 RME 扩展，并且启用了 GPC，则在 Root 世界中运行的软件需要首先初始化 GPT。
...
SMMU 是否支持发出一致性访问由 *SMMU_IDR0.COHACC** 指示。
 ...
 > **注意**
 > 
 > [Arm 基础系统架构](https://developer.arm.com/documentation/den0094/latest/) 需要 I/O 一致性转换表和配置结构访问。

为了支持 I/O 一致性访问，SMMU 需要一个提供正确一致性保证的互连端口。例如，AMBA 互连中的 ACE-Lite 端口可以支持 I/O 一致性访问。
 ...
 * 如果分别通过 **CD.TBI0** 或 **CD.TBI1** 使用顶部字节忽略，则 **VA[55:N-1]** 全部相同。
...
![包括具有非一致性缓存的加速器的系统拓扑简化示例](images/1315506-90f4fdcd8ff3ec28.png)
...
![包括具有一致性缓存的 CXL 加速器的系统拓扑简化示例](images/1315506-317e65eb577464d3.png)
